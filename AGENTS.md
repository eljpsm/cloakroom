...
A minimal CLI for context-dependent Git identities. It compiles
`~/.config/cloakroom/config.toml` into native Git conditional includes, so Git
picks the right `user.name` and `user.email` per repository without cloakroom
running.
